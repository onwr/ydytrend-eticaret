...
export function DesktopNavigation({
  ...
  return (
    <nav aria-label="Kategoriler" aria-busy={navLoading} className="relative z-40 hidden border-t border-brand-border md:block">
            ...
            displayItems.map((item) => {
              ...
              return (
                <li
                  ...
                  onMouseEnter={() => (item.children?.length ? handleEnter(item.id) : undefined)}
                ...
                >
                  <Link
                    ...
                    onClick={() => {
                      if (item.children?.length) onToggleMenu(item.id)
                    }}
                  ...
                  <AnimatePresence>
                    {activeMenu === item.id && item.children && item.children.length > 0 && (
                      <motion.div
                        ...
                        onMouseLeave={handleLeave}
                        className="absolute left-0 top-full z-50 min-w-full w-max pt-0"
                      >
                        <div className="h-1" />
